...
// Main import function - TEST VERSION (only first 10 rows)
async function testImportSmall() {
  const client = await pool.connect();
  
  try {
    console.log('🧪 TESTING: Starting small import (first 10 rows)...');
    ...
    await new Promise((resolve, reject) => {
      stream.pipe(csv())
        .on('data', (data) => {
          ...
          results.push(data);
          if (results.length >= 10) { // Only process first 10 rows
            console.log('🛑 Reached 10 rows, stopping...');
            shouldStop = true;
            stream.destroy();
            resolve();
          }
        ...
        .on('end', async () => {
          if (!shouldStop) {
            try {
              console.log(`📊 Processing ${results.length} rows (TEST MODE - limited to 10)`);
...
module.exports = { testImportSmall };
