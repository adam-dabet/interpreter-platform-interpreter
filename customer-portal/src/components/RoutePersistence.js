...
const RoutePersistence = () => {
  ...
  useEffect(() => {
    if (isAuthenticated && !isLoading) {
      localStorage.setItem('customerCurrentRoute', location.pathname);
    }
  ...
  useEffect(() => {
    if (isAuthenticated && !isLoading) {
      const savedRoute = localStorage.getItem('customerCurrentRoute');
      
      // Only restore if we're not already on a valid route
      if (savedRoute && savedRoute !== '/' && savedRoute !== '/dashboard') {
        // Check if the saved route is valid (not login/verify pages)
