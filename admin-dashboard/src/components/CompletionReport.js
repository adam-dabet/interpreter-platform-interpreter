...
const CompletionReport = ({ jobId, jobData, onSubmit, onCancel }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE}/admin/jobs/${jobId}/completion-report`, {
        method: 'POST',
        headers: {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            {jobData?.job_ended_at && (
              <p className="text-xs text-gray-500 mt-1">
                Job ended at: {formatTime(jobData.job_ended_at)}
              </p>
            )}
          </div>
        </div>
        ...
        {jobData && (
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm text-blue-800">
              ...
              <div>
                <span className="font-medium">Service Type:</span> {jobData.service_type_name || 'N/A'}
              </div>
            </div>
          </div>
        )}
